<script>
  ...
  function checkoutSingle() {
    ...
    if (correct) {
      ...
      const paymentType = company ? company.payment_type : "per_job";
      if (paymentType === "per_job") {
        recordPerJobPayout(totalEarnings, selOrders[0].id);
      } else {
        updateHourlyEarned();
      }
      $uniqueSets += 1;
      completeOrder(selOrders[0].id, {
        ...
        paymentType,
        payout: paymentType === "per_job" ? totalEarnings : 0,
      });
  ...
  function checkoutBundle() {
    ...
    if (correct) {
      ...
      let payoutTwo = 0;
      if (paymentType === "per_job") {
        const baseTotal = selOrders[0].earnings + selOrders[1].earnings;
        if (baseTotal > 0) {
          payoutOne = roundToCents(
            totalEarnings * (selOrders[0].earnings / baseTotal),
          );
          payoutTwo = roundToCents(
            totalEarnings * (selOrders[1].earnings / baseTotal),
          );
        } else {
          payoutOne = roundToCents(totalEarnings / 2);
          payoutTwo = roundToCents(totalEarnings / 2);
        }
        recordPerJobPayout(payoutOne, selOrders[0].id);
        recordPerJobPayout(payoutTwo, selOrders[1].id);
      } else {
        updateHourlyEarned();
      }
      $uniqueSets += 1;
      completeOrder(selOrders[0].id, {
        companyId: company ? company.id : null,
        paymentType,
        payout: paymentType === "per_job" ? payoutOne : 0,
      });
      completeOrder(selOrders[1].id, {
        companyId: company ? company.id : null,
        paymentType,
        payout: paymentType === "per_job" ? payoutTwo : 0,
      });
      $finishedOrders.push(selOrders[0]);
...
<div class="bundlegame max-w-4xl mx-auto px-6 py-4 space-y-4 text-gray-800">
  ...
  {#if GameState == 0}
    ...
    {#if $game.bundled}
      <button
        class="bg-green-500 hover:bg-green-600 text-white font-semibold px-4 py-2 rounded transition"
        id="startbundle"
  ...
  {:else if GameState == 1}
    ...
    <div>
      <div class="flex items-center gap-2 mb-4">
        ...
        />
        Quantities:
        <input class="border rounded px-2 py-1 w-8" bind:value={bag1Input} />
        {#if $game.bundled}
          <input class="border rounded px-2 py-1 w-8" bind:value={bag2Input} />
        {/if}
        <button
          id="addtobag"
          class="bg-blue-500 hover:bg-blue-600 text-white font-medium px-3 py-1 rounded"
        ...
        {#each config["locations"] as row, rowIndex}
          {#each row as cell, colIndex}
            {#if rowIndex == curLocation[0] && colIndex == curLocation[1]}
              <button
                id="moveinstore"
                class="cell rounded-md border text-sm p-3 bg-gray-100 hover:bg-gray-200 transition"
                class:selected={rowIndex == curLocation[0] &&
                  colIndex == curLocation[1]}
                on:click={() => handleCell(cell, rowIndex, colIndex)}
              >
                {cell}
                <p class="text-lg">{emojis[cell]}</p>
              </button>
            {:else}
              <button
                id="moveinstore"
                class="cell rounded-md border text-sm p-3 bg-gray-300 hover:bg-gray-400 transition"
                class:selected={rowIndex == curLocation[0] &&
                  colIndex == curLocation[1]}
                on:click={() => handleCell(cell, rowIndex, colIndex)}
              >
    ...
    {#if $game.bundled}
      <button
        class="mt-4 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded transition"
        id="checkout"
  ...
  {:else if GameState == 3}
    ...
    <button
      class="mt-4 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded transition"
      id="ordersuccess"
  ...
  {:else}
    ...
    <button
      class="mt-4 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded transition"
      id="orderretry"
