...
<div class="container mx-auto px-4 py-6">
  ...
  {:else}
    ...
    {#if started}
      <div
        class="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-gray-800"
      >
        <span><span class="font-medium">Time:</span> {$elapsed}s</span>
        <span><span class="font-medium">Earned:</span> ${$earned}</span>
        {#if $currentCompany && $currentCompany.payment_type === "hourly"}
          <span>
            <span class="font-medium">Hourly Earned:</span> ${$hourlyEarnings}
          </span>
          <span>
            <span class="font-medium">Rate:</span>
            ${Number($currentCompany.hourlyRate ?? 0).toFixed(2)}/hr
          </span>
        {/if}
        <span><span class="font-medium">Location:</span> {$currLocation}</span>
        <span>
          <span class="font-medium">Company:</span>
          {$currentCompany ? $currentCompany.name : "None selected"}
        </span>
      </div>
